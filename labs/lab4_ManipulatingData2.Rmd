...
- Logical testing uses using comparison operators like `<` and `==` and logical operators (e.g., `&&` and `||`) to test which elements in our data structure meet specific requirements 
...
* A list is also 1D data structure, but they have a few tricky differences from vectors, so we'll save them until the end.
...
- numeric vectors specificying the position of the rows/columns we want to access
- logical vectors specifying for each column and row whether we want to access it (`TRUE`)  or ignore it (`FALSE`)

## Matrix Examples
Create a small matrix & index it [row,column] style
```{r mat_examples}
...
## Matrix Examples
If you want to select **all** of one dimension, (e.g., keep all rows or all columns) but selectively index the other dimension, provide the separating comma as usual, but don't give any indexing vector for the dimension you want to stay 100% intact.
```{r mat_examples3}
...
```{r mat_examples4}
colSums(dummy) # colSumns() adds up each column
colSums(dummy) < 8 # Does each column sum to less than 8?
dummy[,colSums(dummy) < 8] # Keep columns with sum less than 8
```

## Logicals & 2D structures
This example keeps all the rows of the matrix where the value in the first column is more than 5
```{r mat_examples5}
dummy[,1]
dummy[,1] > 5
dummy[dummy[,1] > 5,] # R drops dimensions by default!
dummy[dummy[,1] > 5, ,drop=FALSE] # R preserves dimensions!
```
...
- Indexing based on the name you've assigned to row or column is almost always better, because that name is unlikely to change, while the row or column number is **very** likely to get changed
- It's also much easier to remember the name of something, than its position in a data frame
...
We can still use names to get what we want by using a character vector containing names of our desired columns.
...
## Errors when indexing by name
Its also a common mistake to forget the quotes around names inside the `[]` brackets, which will throw an unfound object error.
<div class = "errormsg">
...
## Lists
As we learned previously in the Data Structures and Types lessons, lists are the most abstract data structure, capable of holding heterogenous data types as well as holding other data structures. 
...
biglist <- list(training = c(1,3,2,4),
                data = data.frame(trial = 1:4,
                          average = c(.4,.71,.64,.1)),
                times = matrix(c(2.74,3.44,2.91,.65)),
                name = "Herp McDerpsen")
```
...
## Indexing Lists
Rules for indexing lists are like some other data structures, but also have a few of their own. 

- Lists elements can be named and accessed by name, like the columns of a data frame
- Lists are a 1D structure, meaning there is no use for comma-separated sets of vectors for the rows or columns. You can index them like a normal vector
- Possible to index using normal indexing, i.e., `[]` or recursive indexing i.e.,`[[]]`.

## Indexing lists by name 
```{r biglist_by_name}
biglist$data # Returns a matrix
...
biglist[2] # Normal indexing: returns list with 1 element
biglist[[2]] # Recursive indexing: returns a data frame
```

## Activity 
1. Subset the first 3 elements of `biglist` by name 
2. Subset the last element of `biglist` by position, keeping the returned value as a list
3. Extract character value stored in the `name` field of `biglist`
...
## Replacing Values
Lets say wanted to change the some of the names of the pesticides stored in the `spray` column in the `InsectSprays` data frame to be more informative than just A, B C, etc. 
```{r renaming_sprays}
...
2. Find the `is.na()` fuction to find and remove rows that are missing observations in the `Ozone` column.
...
```
So it is quite easy to write code that succesfully executes, but which produces meaningless output! This is much more insidious than an outright error!

## Activity 
Try to reason about what R "is doing" in the previous example. Why do we get the ouput we get?
```{r mat_examples6_2, ref.label= 'mat_examples6'}
```
As a hint, try runing the code "from the inside out", one logical step at a time.
