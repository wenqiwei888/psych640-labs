...
- This is accomplished by specifying the file's location in the file system heirarchy, better known as the file path.
- This can be done in 1 of 2 ways:
    1. With a file's absolute path (i.e. specifying its location relative to the start of the file system)
    2. With a file's relative path (i.e. specifying its location relative to R's current working directory)
...
- On OSX, OS9, the default working directory is your user's directory /Users
- On Linux, the default working directory is your user's directory under /home.
...
getwd()
```
```{r badexamples2_2, eval=FALSE}
print("/home/will")
```

So we can remove the "/home/will" portion of the path, and use the remaing relative portion to read in the file.
```{r badexamples3,eval=FALSE}
read.csv("data/commas.csv")
```

## The Structure of (Good) Data
It's worth begining with an outline of a well-formatted data set. 
...
## Plain Text Formats
- Plain text files consist only of sequences of characters codes, including spaces, tabs, new lines and delimeters.
...
In a csv file, the columns in each row are separated by commas, naturally.
...
--- |  ------- | ------
file | Path where R looks for file | None, `file` must be specified
...
- Specifying the `sep` argument allows you to read in files where the columns are separated by non-standard delimeters
...
- Specifying the `dec` argument allows you to read in data sets were the decimal place is denoted with `,`. 
 - Using `col.names` together with `header=FALSE` allows you to set the column names in the resulting data frame, even when the data file has no column names.
...
Insted of specifying the path to the file you want as a character vector, you instead pass the `file.choose()` function as the first argument.
...
Sometimes you may encounter data where the columns in each row are not separated by a delimeted, but by a fixed number of character spaces.
...
We can see that the first column takes of 1 space, the second column takes up 1 space, and that the third colun takes up 2 spaces.
...
But otherwise he easist way to read in data from .xls and .xlsx files is using the `readxl` package. Get it with `install.packages('readxl')`.

`readxl` is a relatively new package, so if there are new features/bug fixes you want, they may be present in the development version and not in the CRAN version. You can get the dev version with:
...
In general though, CRAN packages are more stable and should be preferred to development versions. 
...
```{r xlsx3, eval=FALSE}
excel_sheets("data/datasets.xlsx", echo = FALSE) # Mostly for interactive use
```
```{r xlsx4, echo=FALSE}
print(sheetnames)
```
...
If we just want a one-off solution for getting the some data out of an Excel workbook, and into R, with minimal thinking, we can use the system clipboad! 

On Windows, there is a base R solution: Highlight the cells you want, copy them to the clipboad (i.e., Ctrl + c), and then in R, call `read.table("clipboard")`. 
...
If you want to write xlsx files to disk, or do more complex tasks utilizing Excel as a back end (e.g. writing a vector to a specific column in an Excel sheet), take a both the `xlsx` package, and the `XLconnect` package.
...
## Reading SPSS Files
We can use the `read.spss()` function to import .sav files from SPSS
```{r spss_NOTRUN,eval=FALSE,prompt=FALSE}
library(foreign)
spss <- read.spss("data/example.sav", to.data.frame=TRUE,
                  use.value.labels=FALSE) # spss values = factors?
```

The `to.data.frame` argument specifies should the imported file be a data frame (if not, it becomes a matrix)

The `use.value.labels` argumment controls whether columns of data in the .sav file with the values attribute set should be imported as a factor (using the value labels as the levels) or if they should be imported as characters/numerics. 

## Reading in SAS files
If you have SAS data files you want to import into R, use the functions in the `sas7bdat` package.

```{r sasexamples,eval=FALSE}
install.packages("sas7bdat")
library(sas7bdat)
mySASData <- read.sas7bdat("data/randomSASfile.sas7bdat")
```
...
## Reading in MATLAB files
To supress printing of many extraneous details about the .mat file, set it's attributes to `NULL` in R
```{r mfiles2}
