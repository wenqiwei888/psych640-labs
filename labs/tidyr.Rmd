...
Basically, this means is that when the column(s) given in the `value` argument are removed, the rows of your data frame are not unique. Thus, `spread` can't figure out which observations belong in each row when it reshapes that data. Rather than potentially corrupt your data, it gives up instead.
