...
When you add, divide, multiply, or subtract a set of vectors, R matches the vectors up by postition and applies the operation to each pair of elements. 
```{r math_vectors}
vec1 <- c(5,-1,100,75)
vec2 <- c(10,3,4,-4)
vec2 + vec1
...
## Matrix Math
When doing math on matrices with the `+`, `*`, `/` or `-` operators, R will by default applies the operations element-wise. But you can do matrix multiplication with the `%*%` operator. 
```{r linear_alg}
long %*% wide
```
