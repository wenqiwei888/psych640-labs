...
typeof(as.integer(2)) # Same as L suffix
typeof("herp")
typeof(FALSE)
```
...
Of course, it is helpful in a language focused on analysing large data sets to have data structures capable of holding more than a single value.

Think of data structures in R as big containers used for grouping together many values. After storing your data in these containers, you can reuse it multiple places (e.g. create an R object to store it) or access different subsets of it by postion or name.
...
The different values themselves in the vector are referred to as *elements*, and vectors have a *length* equal to the number of elements it contains. 

Vectors are the most basic data structures in R. In fact, all the scalar values we have worked with so far are represented internally by R as stored in vectors of length 1!!

## Creating a vector

Vectors, no matter what type of data they hold, can be created by using the `c()` function in R, short for *concatenate*. Just separate each value you want to be in the vector inside the parenthesis, separated with a comma.
...
`c()` can combine vectors stored as R objects in environment, not just create new one 
```{r concatenate}
...
When you add, divide, etc. vectors, R matches them up by postition and applies the operation to each pair of elements. 
```{r math_vectors}
vec1 <- c(5,-1,100,75)
vec2 <- c(10,3,4,-4)
vec2 + vec1
...
## Summarizing Vectors
Often it is useful to summarise a vector with a single number, and many common statistical operations in R are implemented as function that take vector input. 
...
## Matrices
Matrices in R are the same as matrices from linear algebra, with the notable difference of being able to hold non-numeric types of data. They are a rectangular 2D data structure, meaning that the number of elements in the matrix must be equal to the product of the number of rows and the number of columns.  

- rows = dimension 1
- columns = dimension 2

Do note that data types may not be mixed in a matrix (e.g. cannot have some elements be characters and other be numeric, etc.)

## Creating Matrices
A matrix can be created by feeding a vector into the `matrix()` function, and specifying either the number of rows *or* number of columns, *or* both. 
```{r makeMatrix}
...
## Matrix Math
When doing math on matrices with the `+`, `*`, `/` or `-` operators, R will by default applies the operations element-wise. But you can do matrix multiplication from linear algebra with the `%*%` operator. 
```{r linear_alg}
long %*% wide
```

## Matrix Indexing
The elements of a matrix can be individually accessed via indexing with the `[` operator. The `[` operator is combined with a (row,column) subscript or a linear position index. 
```{r indexing_matrices}
long[1,2] 
long[3,] 
wide[,2]
...
## Data Frames
Data Frames are similar to matrices, in that they are 2D rectangular structures, but are more flexible because they allow for mixed data types. A Data Frame is usually the best way to store and work with data in R that mixes qualitative and quantitative variables. 

Data frames also support a more powerful syntax for subsetting and combining data than matrices, making them a prefered data structure in many instances where explicit matrix algebra is not required. 

## Creating Data Frames
Data frames can be created by passing `name = value` pairs to the `data.frame()` function. The values should be vectors (of any type) and the names should be unquoted character strings, which will be used to name the columns of the data frame. Using numeric values to name the columns (e.g. 1) is possible, but discouraged.
...
```
More than 1 name doesn't work here though. We'll see a way to get more than 1 column next wek
...
df
```
But unlike with the `cbind` example, we did assigmnet here, so the column got added instead of jsut printed. 

## Growing Data Frames
Rows can be added with rbind (guess what thats short for...)
```{r rbind_example}
...
## Data Frame Indexing
Data Frames can also be indexed with `[` like a matrix. 
```{r square}
df[1,] # Still A data frame
df[,3] # A vector, like when using $
```
...
## Lists
Lists are the most abstract and flexible data structure in R. Lists can hold any type of R object, but doesn't impose any relationship between them. You can have a list holding matrices, data frames, vectors, and even other lists!
...
## Creating Lists
As you might have guessed, you can create lists of your own with the `lists()` function. Like the `data.frame()` function, you pass in name=value pairs. But now, the values can be any R object, not just vectors.
...
## Indexing Lists
Like data frames, lists can be indexed with `$`. But, `$` only works if the list items have names. 
```{r biglist_index}
biglist$first
```

You can use `[` with a numeric position index also, but its trickier - we'll talk about it next time
...
2. report the details of this lists struture, and the structure of the objects it holds, using an R function. 
