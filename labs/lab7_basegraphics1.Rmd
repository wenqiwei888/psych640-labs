...
- You can manipulate other aspects of your plot's appearance in the function call to `plot()`, even though they are not listed in the **Usage** section
    - This is because `plot()` passes named arguments that don't match its own list of arugments onto the common graphics settings function, via that `...`
...
pch | The symbols used for points | Numeric vector (see `?points`) [Cheat Sheet](http://vis.supstat.com/2013/04/plotting-symbols-and-color-palettes/)
...
## Adding points
- Not suprisingly, the R function which can draw new points is called `points`
