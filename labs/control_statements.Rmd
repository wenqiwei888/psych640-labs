...
```
- `condition` is an R expression that evaluates to a single logical value (i.e. a single `TRUE` or `FALSE` value)
...
This counter variable (`i` in this example) is declared inside the parenthesis immediately following the `for` keyword.

It is common to refer to the counter variable as the variable you "loop over".

Here, we loop over the variable `i`. Inside the body of the `for` loop, the variable `i` takes on the values 1, 2, and 3.
